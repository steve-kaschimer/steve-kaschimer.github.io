...
| Editorial calendar | `editorial-plan.md` - the single source of truth for what content is planned/drafted/published, organized into monthly batches plus a rolling `Pipeline` section. Each entry tracks `Status` (`idea` → `draft` → `published`), `Scheduled` date, GitHub `Issue`, and `File` once drafted. | `editorial-plan.md` |
...
- This glossary reflects the repo as of 2026-07-12. 
