...
If you're deciding between ORMs first, a comparison of the top .NET ORMs covers where Dapper fits relative to EF Core, NHibernate, Linq2Db, and RepoDb.
...
**Keep SQL close to the code that uses it, but out of business logic.** A dedicated repository or query class per feature (especially natural if you're also using Vertical Slice Architecture) keeps SQL readable without scattering raw strings through unrelated logic.
