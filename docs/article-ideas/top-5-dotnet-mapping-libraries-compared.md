...
- Runtime-based mapping means configuration errors (a typo in a member name, a missing mapping) surface at runtime rather than compile time, a real category of bug the source-generator-based alternatives eliminate entirely
...
Mapster built its reputation as the free, flexible AutoMapper alternative -- a static `Adapt()` extension method that works with zero configuration for simple cases, plus an optional compile-time mode (via Mapster.Tool) for teams wanting source-generated performance without a full syntax change.
...
**Choose this when:** you want a free, low-friction AutoMapper replacement with minimal migration effort -- particularly as an interim step, registered alongside existing AutoMapper profiles during a gradual migration -- while being realistic about its uncertain long-term development trajectory.
...
- Zero external dependency, zero licensing risk, and completely transparent, debuggable code -- what you see is exactly what runs, with nothing generated or reflected
...
- No automatic handling of common patterns (flattening, unflattening, collection mapping) that a library provides out of the box -- you're reimplementing those patterns yourself each time
...
**Choose this when:** your mapping surface is small to moderate, your team values maximum transparency and zero dependency risk over convenience, or you're working in a context (a library, a package meant for broad consumption) where adding a mapping dependency isn't appropriate.

## Facet

Facet represents a different framing of the whole problem: instead of asking "how do I map an existing DTO from my domain model," it asks "why am I hand-writing the DTO at all." Using C# source generators, Facet generates both the destination type (a "facet" of your domain model showing only the fields relevant to a specific view) and the mapping code to populate it, from a single declarative attribute.
...
- Emerged directly in response to the gap AutoMapper's commercial shift and the broader source-generator trend created, reflecting where a meaningful part of the ecosystem's thinking has moved
...
- Much smaller community and real-world adoption than any of the other four -- genuinely early-stage relative to this comparison's other entries
...
Yes, and this is a deliberate pattern some teams use -- retaining AutoMapper specifically for EF Core `ProjectTo` projection (where it remains the most mature option) while routing all other in-memory mapping through Mapperly. It's not a transitional accident; it's using each tool for what it's genuinely best at.
