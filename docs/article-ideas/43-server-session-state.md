...
csharp: 14
description: Keep conversational state on the application side while
  clients carry only a session identifier, using ASP.NET Core session
  and distributed caches.
dotnet: 10
fowler_url: "https://martinfowler.com/eaaCatalog/serverSessionState.html"
order: 43
pattern: Server Session State
