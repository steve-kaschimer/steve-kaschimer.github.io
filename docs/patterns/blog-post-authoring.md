...
10. Hero image is a separate, later step - see `docs/decisions/2026-07-12-hero-images-are-a-manual-handoff.md`. A post ships and can even go live without one; it falls back to a gradient header/card. Once the maintainer hands back a PNG generated from `image_prompt`, convert it to the standard asset set:
    ```bash
