...
- Passthrough copy: `src/images/**`, `src/js/**`, and `src/downloads/**` go straight to `_site` unprocessed
...
Front matter used across templates: `layout: post.njk`, `title`, `author`, `date` (YYYY-MM-DD - this is what gates publish/visibility and sort order, independent of the filename), `image` (path under `/images/posts/`, `.webp`; the homepage card derives a responsive `srcset` by swapping in `-400w`/`-600w`/`-800w` filename suffixes), `image_alt` (alt text for the hero image on the post page, via `post.njk`'s `{{ image_alt | default('') }}` - describe what the image actually depicts, not the post topic. Not used on the homepage card: `index.njk` hardcodes `alt=""` there since the card's image is decorative next to the post title/summary text that already conveys the same information), `image_prompt` (documents the AI image-gen prompt for the hero - not rendered, kept for provenance), `summary` (used for the card excerpt, meta/OG description, and RSS), `tags` (array - drives the tag filter UI and RSS `<category>` entries), `site_title`.

Optional companion-download fields (introduced for the Modern Application Architecture Patterns in .NET series, volume 3, which ships alongside a runnable Northstar Architecture Lab codebase): `companion_download` (absolute path under `/downloads/`, e.g. `/downloads/northstar-baseline.zip`) and `companion_download_label` (display text for the download button, e.g. `"the baseline application"`). When `companion_download` is set, `post.njk` renders a "Companion Source Code" `callout-box` after the article body with a download link - both fields are optional and only meaningful together. Downloadable archives live in `src/downloads/` (passthrough-copied to `_site/downloads/` via `.eleventy.js`, same mechanism as `src/images`/`src/js`) - keep them off `npm run deploy`'s critical path and don't check in anything there that isn't actually linked from a post.

Front-matter keys are kept alphabetized by convention (`author`, `companion_download`, `companion_download_label`, `date`, `image`, `image_alt`, `image_prompt`, `layout`, `site_title`, `summary`, `tags`, `title`).

Filenames follow `YYYY-MM-DD-slug.md`; that date is just for ordering files on disk - the front-matter `date` is authoritative.

**Hero images:** this environment's outbound network policy blocks direct calls to `api.openai.com`, so image generation can't be automated from inside a Claude Code session here. When a post is drafted without a hero image, remind the user to generate one from the post's `image_prompt` field using **ChatGPT** (their preferred tool), then hand the resulting image back so it can be converted into the standard asset set (`<slug>-hero.png` source, full-size `.webp`, plus `-400w`/`-600w`/`-800w` responsive variants at quality 95, matching the existing posts) using ImageMagick's `convert` (full command sequence: `docs/patterns/blog-post-authoring.md`). Write the post's `image_alt` field at the same time - describe what's actually depicted in the generated image, not a restatement of the post's title or topic.
...
- `post.njk`: extends `base.njk`. Renders a hero - full-bleed image with `<picture>`/webp source if `image` is set, otherwise a gradient fallback - then post content inside `.prose`, then (if `companion_download` is set) a "Companion Source Code" callout box with a download link, then a "Back to all posts" link.
