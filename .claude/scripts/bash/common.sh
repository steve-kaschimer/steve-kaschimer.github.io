...
# Safely read .claude/feature.json's "feature_directory" value.
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.claude/feature.json"
...
# Returns 0 when .claude/feature.json lists feature_directory that exists as a directory
...
get_feature_paths() {
    ...
    #   2. .claude/feature.json "feature_directory" key (persisted by /speckit.specify)
    ...
    elif [[ -f "$repo_root/.claude/feature.json" ]]; then
...
#   1. .claude/templates/overrides/
#   2. .claude/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .claude/extensions/<ext-id>/templates/
#   4. .claude/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.claude/templates"
    ...
    local presets_dir="$repo_root/.claude/presets"
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.claude/extensions"
    if [ -d "$ext_dir" ]; then
