...
date: 2027-01-26
image: /images/posts/2027-01-26-hero.webp
image_alt: "A padlock icon on a class-level box beside an unlocked icon on a per-test box, with two overlapping platform outlines and a short arrow moving from the older one to the newer one."
image_prompt: "A dark-mode technical editorial illustration on a near-black background with electric teal, amber, and off-white accents. Composition centers on two flat rectangle outlines side by side: the left one, labeled implicitly as class-level, has a small amber padlock icon in its corner; the right one, labeled implicitly as per-test, has a small open-lock icon in teal. Below, two overlapping rounded rectangle platform shapes sit slightly offset from each other, with a short curved arrow pointing from the rear one to the front one, suggesting a gradual migration between two execution platforms. Mood is procedural, official, and deliberate. Avoid: vendor logos, brand colors, circuit-board textures, gears, or any Microsoft/Visual Studio iconography."
layout: post.njk
site_title: Tech Notes
summary: "MSTest's feature gap with xUnit and NUnit has narrowed considerably - the decision that matters now is VSTest vs. Microsoft.Testing.Platform. A setup guide for the static class-level lifecycle and the platform choice the project file doesn't make obvious."
...
If you're deciding between testing frameworks first, [a comparison of the top .NET testing frameworks](/posts/2027-01-05-top-5-dotnet-testing-frameworks-compared/) covers where MSTest fits relative to xUnit, NUnit, TUnit, and Expecto.
