...
One of the easiest ways to make a new system unnecessarily difficult is to begin with:
> How many microservices should we have?

That question is premature. Start with:
...
Those may be meaningful modules or bounded contexts. They do not automatically need to be separate processes. A modular monolith can preserve boundaries inside one deployment:
``` text
┌───────────────────────────────┐
│        ASP.NET Core App       │
│                               │
...
The important property is that modules do not become one undifferentiated codebase.
...
The call is no longer a method call. Now:
``` text
...
Microservices do not merely rearrange projects. They convert in-process coupling into distributed-systems problems.

## The Database Boundary

A useful microservice boundary generally includes ownership of data. This:
``` text
Order Service ----Billing Service ---+--> Shared Database
Shipping Service -/
```

may provide independent processes without meaningful autonomy. Shared schemas create coupling through:
-   migrations;
-   transactions;
-   queries;
-   data semantics;
...
If services cannot change their data independently, the service boundary may be weaker than it appears.

## Module Contracts

A modular monolith can prohibit arbitrary cross-module access. For example:
``` text
Ordering
   |
   v
...
or communicate through internal events. The exact mechanism matters less than preserving the rule:
...
A well-designed logical boundary can make the physical split less traumatic. It does not make extraction free. Distributed communication still changes consistency and failure semantics.
...
"We expect lots of users" is not enough by itself. A monolith can scale horizontally too.

## Organizational Architecture

Service boundaries often become team boundaries. That means architecture and organization interact. If five teams constantly modify the same service, the service may not represent a useful ownership boundary. Conversely, creating twenty services for a five-person team can impose operational coordination the organization cannot sustain.
...
Modularity and microservices solve different problems. Modularity is about boundaries. Microservices add process and deployment independence. A modular monolith often gives a system the first benefit without immediately accepting the network, consistency, observability, and operational costs of the second. Find the boundary first. Then decide whether that boundary deserves its own process.
