...
Applications fail in different ways. These are not the same:
```text
...
`null` tells us only that there is no Order. It cannot naturally distinguish:
```text
not found
not authorized
...
Generic `Result<T>` is not mandatory. Sometimes:
```csharp
...
Some domain models protect invariants by throwing domain exceptions. Others return domain results. Both can work. For example:
```csharp
...
The endpoint translates application outcomes into transport semantics. The application does not need to return `IResult`.
...
inside the application layer. HTTP is an adapter concern. Instead:
```text
Application Result
       |
HTTP Adapter
...
The human message can change. The code can remain part of a contract. Do not expose internal exception type names as public error codes.
...
If understanding the success path requires decoding a type puzzle, the abstraction has failed. Domain-specific result types and ordinary exceptions are often clearer.

## Logging

Expected failures should not all be logged as errors. A declined card is business information. A database outage is an operational error. Result modeling helps telemetry distinguish them.
...
Result is a modern application-level companion to patterns such as Special Case and Service Layer. It also pairs naturally with Commands, Queries, Vertical Slices, and HTTP `ProblemDetails`.

## Summary

Result makes **expected failure** explicit. It does not eliminate exceptions. Model ordinary business/application outcomes as values when doing so improves the contract, and preserve exceptions for failures that are genuinely exceptional or operational. The goal is clarity, not a world in which `throw` is forbidden.
---
